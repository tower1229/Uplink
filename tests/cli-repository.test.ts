...
test("status errors are machine-readable and do not expose invalid config contents", () => {
  ...
  assert.equal(response.error.formalRepositoryDataWritten, false);
  assert.match(response.error.recoveryAction, /uplink rebind <path>/i);
...
test("rebind requires explicit confirmation before switching to a valid Repository", () => {
  ...
  const firstRepository = path.join(sandbox, "first");
  const secondRepository = path.join(sandbox, "second-$(do-not-run)");
  ...
  assert.match(refusal.error.message, /does not migrate data/);
  assert.equal(
    refusal.error.recoveryAction,
...
test("human-readable rebind failures include the operation id and write marker", () => {
  const sandbox = mkdtempSync(path.join(tmpdir(), "uplink-rebind-human-error-"));
  const repositoryPath = path.join(sandbox, "repository");
  const configDirectory = path.join(sandbox, "config");
  mkdirSync(repositoryPath);
  invokeCli(["init", "--json"], repositoryPath, configDirectory);
  ...
  const attempt = spawnSync(
    process.execPath,
    [tsxPath, cliPath, "rebind", repositoryPath],
    {
      cwd: sandbox,
      encoding: "utf8",
      env: { ...process.env, UPLINK_CONFIG_DIR: configDirectory },
  ...
  assert.equal(attempt.status, 1);
  assert.match(attempt.stderr, /Event: rebind\.failed/);
  assert.match(attempt.stderr, /Operation ID: op_[0-9a-f-]{36}/);
  assert.match(attempt.stderr, /Formal Repository data written: false/);
  assert.match(attempt.stderr, /--yes/);
});

test("status reports a missing Binding with rebind recovery and no write", () => {
...
test("rebind rejects invalid targets and never migrates Repository data", () => {
  ...
  assert.match(invalidResponse.error.recoveryAction, /uplink rebind <path>/i);
